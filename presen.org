...
+ 最適性/理論保証の求められるミッションクリティカルシステム :: 製造システム、運送、人工衛星(燃料使いきれば運用終了)
...
*棋譜なし* : 棋譜から学習した勘(評価関数)で解くAlphaGoと異なる → *下界関数* を用いる

*事前知識なし* : 「パネル」「9マスある」「パネルが動く」などのラベル、シンボルなし
...
→ ドメイン非依存下界関数による枝刈りが、この未知のドメインでも有効

→ より大規模な問題(15パズル,24パズル等)では *知識なし探索は指数爆発*

#+BEGIN_SMALLER
|------------------------+----------+--------------------------|
| 探索ノード数           | Dijkstra | A*+PDBヒューリスティクス |
|------------------------+----------+--------------------------|
| MNIST 8-puzzle         |   193924 | *109096*                 |
|------------------------+----------+--------------------------|
| MNIST 8-puzzle         |   201156 | *111642*                 |
|------------------------+----------+--------------------------|
| MNIST 8-puzzle         |   186767 | *84561*                  |
|------------------------+----------+--------------------------|
| MNIST 8-puzzle         |   183336 | *82518*                  |
|------------------------+----------+--------------------------|
| MNIST 8-puzzle         |   169907 | *52084*                  |
|------------------------+----------+--------------------------|
| MNIST 8-puzzle         |   130863 | *26967*                  |
|------------------------+----------+--------------------------|
| Hanoi (4 peg)          |       55 | *17*                     |
|------------------------+----------+--------------------------|
| LightsOut (4x4)        |      952 | *27*                     |
|------------------------+----------+--------------------------|
| Spiral LightsOut (3x3) |      522 | *214*                    |
|------------------------+----------+--------------------------|
| Mandrill 8-puzzle      |   335378 | *88851*                  |
|------------------------+----------+--------------------------|
#+END_SMALLER

#+BEGIN_LARGER
注: *プランニングでは理論保証付き下界関数のことをヒューリスティクスと呼ぶ*
#+END_LARGER
...
+ 主な貢献: *SAEによって生データからシンボルを生成してプランニング問題が解ける* ことを実証
...
ニューラルネットが収束していない場合 (収束が保証されているのは $t\rightarrow \infty$ のみ, not 実時間)

 → でたらめなシンボル/グラフが生成

 → でたらめなグラフ上の正しいプランが生成 (アルゴリズムの完全性、健全性、最適性により保証)

 → でたらめな画像プラン

#+BEGIN_CENTER
#+BEGIN_LARGER
*認識の間違いはある* が *判断の間違いはない!*
#+END_LARGER
#+END_CENTER
...
+ テキスト用のAE [Li et.al. 2015]
+ 音声用のAE [Deng, Li, et al. 2010]
...
+ 棋譜なし・事前知識なし、訓練画像のみで任意の問題を解くAIシステムを作った
+ State AutoEncoder(SAE): Gumbel-Softmaxを用いたVAE で 記号に変換できる!
+ 主な貢献: *SAEによって生データから生成したシンボリック表現を用いてプランニング問題を解けることを実証*
+ */限りなく単純に作った/* プロトタイプ → 記号/非記号AI分野の様々な技術を取り入れることで */より発展/*
  + 命題の学習 → 述語の学習
  + アクションの汎化
  + 音声やテキストに対しても今後発展させる
...
+ 理解する必要がない ::
     ...
     ドメイン非依存ソルバの顕著な例: *mystery* と *nomystery* ドメイン --- シンボル名だけ意味不明にした運送ドメイン

     (名前を見て特殊アルゴリズムを使う不正を禁じるため)
  
+ 組み合わせが可能 :: 
     Latent vector は 複数の命題の連言 (and)

+ ノイズを含む画像はそのままではシンボルになれない ::

     なんらかの特徴を抽出したものでなくてはいけない
     
     全く異なる画像になる

* Arxiv版を上げた際のTwitterでの議論

*/AlphaGo チーム全員: 「古典プランニング超大事!」/*

 [[png:twitter]]

* 日本のプランニング研究者は */深層学習研究者よりさらに少ない/*
