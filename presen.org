...
DQN = Subsymbolic (DLNN) + Reinforcement Learning
...
+ 2. 取り出せたとする。NNの出力は Subsymbolic: *実数値*

  + プランナが必要とするのは Symbolic な入力: *離散値・命題・真偽値*

  + Subsymbolic を Symbol をどう変換する? (*Symbol Grounding*) ← *自明でない*

+ 3. 出来たとしても、機械が作った Symbol は *人間には意味不明* 

  + *意味不明のSymbolをどうやって人の理解できる形式にするか？* ← *自明でない*
...
*ニューラルネットの誤差が大きい場合のみ発生* (収束保証は $t\rightarrow \infty$ のみ, not 実時間)
 ...
 → *でたらめな離散グラフ上の正しいプラン* が生成 / *グラフが非連結*
...
+ 理解する必要がない ::
     ...
     ドメイン非依存ソルバの顕著な例: *mystery* と *nomystery* ドメイン

      --- シンボル名だけ意味不明にした運送ドメイン
