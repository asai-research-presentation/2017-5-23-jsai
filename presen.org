...
+ *Do you /really/ believe that humanity is /so/ smart?*
#+END_SPAN8
#+BEGIN_SPAN4
[[sjpg:trump]]
...
#+END_SPAN4
#+BEGIN_SPAN12
  + *The sad reality: People are not necessarily smart.↑*
#+END_SPAN12
#+END_ROW-FLUID
#+END_CONTAINER-FLUID
...
+ 第二次AIの冬の原因:
  + *論理だけで全てが解決できると考えた*
  + 現実世界 (物体、アクション) のラベル付を手入力する必要があった
  + ソルバも遅すぎた
+ 現在: より速いソルバがあり、ラベル付もDLでできそうだ
#+END_SPAN6
#+END_ROW-FLUID
#+END_CONTAINER-FLUID

+ 第三次AIの冬のありそうな原因: *学習で全てが解決できると考えた?*
  + 脳科学(笑)とかの迷信に踊らされている日本の傾向
...
論理推論に基づくAI: *棋譜から学習した勘で解くAlphaGoと異なる*
...
* /Manifold Hypothesis/ マニホールド仮説 and /Latent Space/
...
* Gumbel-Softmax (Jang, Gu, ICRA2017に *投稿中*, 2/3に採択結果判明)
...
+ Common Lisp における bit-vector : C における short 配列に相当
  ...
  + コンパイラが64ビット単位の整数処理に自動変換してくれる(のでCと同様に速い)
...
z が与えられていれば、 結果を z に破壊的に代入しそれを返す
...
(LET* ((#:LENGTH (LENGTH C))
       (#:G746 C))
  (LET* ((#:G747 (MAKE-BIT-VECTOR #:LENGTH)))
    (DECLARE (DYNAMIC-EXTENT #:G747)) ; 中間結果G747はスタック配置
    (BIT-AND A B #:G746)
    (BIT-NOT A #:G747)
    (BIT-AND #:G747 C #:G747)      ; 中間ベクトルG747再利用
    (BIT-IOR #:G746 #:G747 #:G746) ; 結果ベクトルG746再利用
    #:G746))
#+END_SRC
