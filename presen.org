...
探すべきプランの始点と終点にある2つの状態(初期状態とゴール状態) を示す画像
...
+ *SAE の実装を変えれば(原理的には)画像以外の任意の入力データに対応可能なはず*
  ...
  + 学習してプランナで解くことにより *数千ステップの高速な言語レベル推論が可能に*

* 今日のメッセージ

#+BEGIN_CENTER
NLPエキスパートと 探索の専門家の力を合わせて このシステムを作ってみませんか？

主な産業応用例: *ルールベース人口無能を超える 論理を備えた 真に知的なチャットボット*
#+END_CENTER
