...
+ 人が不可能な作業の代替 :: 危険な環境, 宇宙空間・深海, 24時間対応
+ 最適性/理論保証の求められるミッションクリティカルシステム :: 製造システム、運送、人工衛星運営(最も少ない燃料で運用)
...
#+BEGIN_LARGER
#+BEGIN_ALIGNRIGHT
非古典的なさまざまな拡張がある (時間,POMDP,HTN... どのAIの教科書にものっている)
#+END_ALIGNRIGHT
#+END_LARGER
...
+ 殆どのAtariゲームは *脊髄反射的な操作で生き残ことが出来る* → *複雑な論理思考能力はいらない!*
...
+ なぜ全結合?? ::
             論文の目的は *SAEで命題を作る方針がそもそもうまく行くかどうか*
...
+ LatPlanα: 一般化を行わない ::
     ...
     *→余計な要素を省いて限りなくシンプルに*
     ...
     → 既存の Domain Acquisition 学習機をそのまま使えば良い (Future work)
...
#+BEGIN_LARGER
注: *プランニングでは下界関数のことをヒューリスティクスと呼ぶ*
#+END_LARGER
